...
public class Main {
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < strategyTArray.length; i++) {
				...
				StrategyLayout strategyLayout = strategyTArray[i]
						.getStrategyLayout();
				StrategyPanel[] strategyPanelArray = strategyLayout
	...
	private static void expandStrategyPanel(Composite parent,
			StrategyPanel[] strategyPanelArray) {
		...
		for (int i = 0; i < strategyPanelArray.length; i++) {
			...
			if (strategyPanelTitle == null
					|| "".equals(strategyPanelTitle.trim())) {
			...
			PanelOrientationT.Enum strategyPanelOrientation = strategyPanel
					.getOrientation();

			if (strategyPanelOrientation.equals(PanelOrientationT.HORIZONTAL)) {
				groupLayout.type = SWT.HORIZONTAL;
			} else if (strategyPanelOrientation
					.equals(PanelOrientationT.VERTICAL)) {
			...
			StrategyPanel[] innerStrategyPanelArray = strategyPanel
					.getStrategyPanelArray();
