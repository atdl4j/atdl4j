...
import br.com.investtools.fix.atdl.ui.swt.PanelFactory;
import br.com.investtools.fix.atdl.ui.swt.ParameterUI;
import br.com.investtools.fix.atdl.ui.swt.ParameterUIFactory;
import br.com.investtools.fix.atdl.ui.swt.WidgetFactory;

public class SWTFactory implements WidgetFactory, PanelFactory {

	private ParameterUIFactory parameterWidgetFactory;

	public SWTFactory() {
		parameterWidgetFactory = new SWTParameterUIFactory();
	}

	@Override
	public ParameterUI<?> create(Composite parent, ParameterT parameter,
			int style) throws XmlException {
		ParameterUI<?> parameterWidget = null;
	...
	public Map<String, ParameterUI<?>> create(Composite parent,
		...
		Map<String, ParameterUI<?>> parameterWidgets = new HashMap<String, ParameterUI<?>>();
		...
		if (panel.getTitle() == null || "".equals(panel.getTitle())) {
			...
			for (ParameterT parameter : panel.getParameterArray()) {
				ParameterUI<?> pw = create(c, parameter, style);
				parameterWidgets.put(parameter.getName(), pw);
			}
			return parameterWidgets;
		}
