...
import org.atdl4j.atdl.core.RepeatingGroupT;
...
public class SWTStrategyUI extends AbstractStrategyUI
{
    ...
    // 2/1/2010
    protected Map<String, RadioButtonListener> radioGroups;
    ...
    public SWTStrategyUI(
	    ...
	    Map<String, ValidationRule> strategiesRules,
	    Composite parent,
	    StrategiesT strategies,
	    Map<String, String> inputHiddenFieldNameValueMap)
	...
	radioGroups = new HashMap<String, RadioButtonListener>();
	...
	if (inputHiddenFieldNameValueMap != null)
	{
	    ...
	    for (Map.Entry<String, String> tempMapEntry : inputHiddenFieldNameValueMap
		    .entrySet())
	    {
	...
	if ((getStrategy() != null) && (getStrategies() != null))
	{
		...
		{
		    builder.onField(getStrategies().getStrategyIdentifierTag()
			    .intValue(), strategyIdentifier.toString());
		} else
		{
		...
		{
		    builder.onField(getStrategies().getVersionIdentifierTag()
			    .intValue(), strategyVersion.toString());
		}
	    }
	}
	...
	/*
	 * 2/1/2010 John Shields added
	 * Beginning of Repeating Group implementation. Currently there is
	 * an error in ATDL I believe where StrategyT can only have one RepeatingGroupT
	HashMap<String, RepeatingGroupT> rgroups = new HashMap<String, RepeatingGroupT>();
	for (RepeatingGroupT rg : strategy.getRepeatingGroup())
	{
	    for (ParameterT rg : strategy.getRepeatingGroup())
	    {
