...
public class ListBoxWidget extends AbstractSWTWidget<String> {
	...
	public Widget createWidget(Composite parent, int style) {
		...
		// listBox items
		java.util.List<ListItemT> listItems = control instanceof MultiSelectListT ? ((MultiSelectListT) control)
				.getListItem()
		...
		// init value
		String initValue = control instanceof MultiSelectListT ? ((MultiSelectListT) control)
				.getInitValue()
		...
		for (String singleValue : values) {
			for (int i = 0; i < getListItems().size(); i++) {
				if (setValueAsControl || parameter == null) {
					...
					if (parameter.getEnumPair().get(i).getWireValue().equals(
							singleValue)) {
						listBox.select(i);
						break;
					}
