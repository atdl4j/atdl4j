...
public class ControlHelper {
    ...
    public static BigDecimal getIncrementValue(ControlT aControl,
	    Atdl4jConfig aAtdl4jConfig) {
	return getIncrementValue(aControl, aAtdl4jConfig, 0);
    }
     ...
     * @return
     */
    // 4/18/2010 Scott Atwell public static BigInteger getIncrementValue(
    // ControlT aControl, Atdl4jConfig aAtdl4jConfig )
    // 8/17/2010 Scott Atwell public static BigDecimal getIncrementValue(
    // ControlT aControl, Atdl4jConfig aAtdl4jConfig )
    public static BigDecimal getIncrementValue(ControlT aControl,
	    Atdl4jConfig aAtdl4jConfig, int aDigits) {
	if (aControl instanceof SingleSpinnerT) {
	    return determineIncrementValue(
		    ((SingleSpinnerT) aControl).getIncrement(),
		    ((SingleSpinnerT) aControl).getIncrementPolicy(),
		    aAtdl4jConfig, aDigits);
	}
    ...
    // 9/11/2010 John Shields adding back previous interface for Swing usage
    public static BigDecimal getInnerIncrementValue(ControlT aControl,
	    Atdl4jConfig aAtdl4jConfig) {
	return getInnerIncrementValue(aControl, aAtdl4jConfig, 0);
    }
     ...
     * @return
     */
    // 4/18/2010 Scott Atwell public static BigInteger getInnerIncrementValue(
    // ControlT aControl, Atdl4jConfig aAtdl4jConfig )
    // 8/17/2010 Scott Atwell public static BigDecimal getInnerIncrementValue(
    // ControlT aControl, Atdl4jConfig aAtdl4jConfig, int aDigits )
    public static BigDecimal getInnerIncrementValue(ControlT aControl,
	    Atdl4jConfig aAtdl4jConfig, int aDigits) {
	if (aControl instanceof DoubleSpinnerT) {
	    return determineIncrementValue(
		    ((DoubleSpinnerT) aControl).getInnerIncrement(),
     ...
     * @return
     */
    // 4/18/2010 Scott Atwell public static BigInteger getOuterIncrementValue(
    // ControlT aControl, Atdl4jConfig aAtdl4jConfig )
    public static BigDecimal getOuterIncrementValue(ControlT aControl,
	    Atdl4jConfig aAtdl4jConfig) {
	if (aControl instanceof DoubleSpinnerT) {
	    return determineIncrementValue(
		    ((DoubleSpinnerT) aControl).getOuterIncrement(),
     ...
     * @return
     */
    public static BigDecimal determineIncrementValue(Double aIncrement,
	    String aIncrementPolicy, Atdl4jConfig aAtdl4jConfig, int aDigits) {
	BigDecimal tempBigDecimal = determineIncrementValue(aIncrement,
		aIncrementPolicy, aAtdl4jConfig);

	if (tempBigDecimal != null) {
	    // 8/23/2010 (handle "Tick" digits 2 - Atdl4jConfig default to .0001
	    // and java.lang.ArithmeticException: Rounding necessary
	    // 8/23/2010 return tempBigDecimal.setScale( aDigits );
	    BigDecimal tempBigDecimal2 = tempBigDecimal.setScale(aDigits,
     ...
     * @return
     */
    // 4/18/2010 Scott Atwell public static BigInteger determineIncrementValue(
    // Double aIncrement, String aIncrementPolicy, Atdl4jConfig aAtdl4jConfig )
    public static BigDecimal determineIncrementValue(Double aIncrement,
	    String aIncrementPolicy, Atdl4jConfig aAtdl4jConfig) {
	// -- FIXatdl 1.1 Schema documentation: --
	// <xs:documentation>
	// Describes how to use increment. If undefined then take value from
	// increment
	// attribute, if LotSize use value based on symbol lot size. (If lot
	// size is not available use value of
	// increment attribute.) If Tick use value based on symbol tick size.
	// (If tick size is not available
	// use value of increment attribute.) If increment is to be used and is
	// not defined then use a system
	// default value.
	// </xs:documentation>
	...
	if (Atdl4jConstants.INCREMENT_POLICY_LOT_SIZE.equals(aIncrementPolicy)) {
	    if ((aAtdl4jConfig != null)
		    ...
		    && (aAtdl4jConfig.getInputAndFilterData()
			    .getInputIncrementPolicy_LotSize() != null)) {
		return aAtdl4jConfig.getInputAndFilterData()
			.getInputIncrementPolicy_LotSize();
	    }
	    // 4/18/2010 Scott Atwell removed
	    // else
	    // {
	    // throw new IllegalArgumentException(
	    // "LotSize for security was not specified.  Unable to support IncrementPolicy="
	    // + aIncrementPolicy );
	    // }
	...
	} else if (Atdl4jConstants.INCREMENT_POLICY_TICK
		.equals(aIncrementPolicy)) {
	    if ((aAtdl4jConfig != null)
		    && (aAtdl4jConfig.getInputAndFilterData() != null)
		    && (aAtdl4jConfig.getInputAndFilterData()
			    .getInputIncrementPolicy_Tick() != null)) {
		return aAtdl4jConfig.getInputAndFilterData()
			.getInputIncrementPolicy_Tick();
	    }
	    // 4/18/2010 Scott Atwell removed
	    // else
	    // {
	    // throw new IllegalArgumentException(
	    // "Tick size for security was not specified.  Unable to support IncrementPolicy="
	    // + aIncrementPolicy );
	    // }
	    else if (aIncrement != null) {
		// 7/18/2010 Scott Atwell return new BigDecimal( aIncrement );
	...
	} else // -- Use aIncrement value when aIncrementPolicy null or
	       // Atdl4jConstants.INCREMENT_POLICY_STATIC --
	{
	    if (aIncrement != null) {
		// 4/18/2010 Scott Atwell replaced return BigInteger.valueOf(
		// aIncrement.longValue() );
		// 7/18/2010 Scott Atwell return new BigDecimal( aIncrement );
		return new BigDecimal(aIncrement.toString());
	    } else {
    ...
    /**
     * Handles ControlT/@initPolicy ("UseValue" or "UseFixField") logic in
     * conjunction with ControlT/@initValue and ControlT/@initFixField Returns
     * null if ControlT/@initValue is the special null indicator:
     ...
     * @return
     */
    public static Object getInitValue(ControlT aControl,
	    Atdl4jConfig aAtdl4jConfig) {
	// INIT_POLICY_USE_VALUE = "UseValue"; // -- use value from
	// ControlT/@initValue --
	// INIT_POLICY_USE_FIX_FIELD = "UseFixField"; // -- use value from
	// ControlT/@initFixField if available, else ControlT/@initValue --

	if (Atdl4jConstants.INIT_POLICY_USE_FIX_FIELD.equals(aControl
		.getInitPolicy())) {
	    if (aControl.getInitFixField() == null) {
		throw new IllegalArgumentException("ERROR: Control: "
			+ aControl.getID() + " has initPolicy=\""
			+ aControl.getInitPolicy()
			+ "\" but does not have initFixField set.");
	    }
		...
		throw new IllegalArgumentException("ERROR: Control: "
			+ aControl.getID() + " has initPolicy=\""
			+ aControl.getInitPolicy()
			+ "\" but Atdl4jConfig is null.");
	    }

	    String tempFixFieldValue = aAtdl4jConfig.getInputAndFilterData()
		    .getInputHiddenFieldValue(
			    aControl.getInitFixField().toString());
	...
	// -- initPolicy is INIT_POLICY_USE_VALUE or no input field value
	// supplied/found for ControlT/@initFixField --
	...
	return (aControl.getLabel() != null && !"".equals(aControl.getLabel())) ? aControl
		.getLabel() : aControl.getID();
    }
     ...
     * conjunction with ControlT/@initValue and ControlT/@initFixField Returns
     * null if ControlT/@initValue is the special null indicator:
     * VALUE_NULL_INDICATOR Also has special handling for LabelT which may
     * simply use "label=" vs. "initValue=" for its value.
     * 
     * @param aControl
     * @param aAtdl4jConfig
     * @return
     */
    public static Object getReinitValue(ControlT aControl,
	    Atdl4jConfig aAtdl4jConfig) {
	Object tempInitValue = getInitValue(aControl, aAtdl4jConfig);

	if ((tempInitValue == null) && (aControl instanceof LabelT)) {
	    // 3/14/2010 Scott Atwell LabelT may simply use "label=" and do not
	    // want to lose it upon reinit
    ...
    /**
     * Returns the default number of Spinner control "digits" for the specified
     * aParameter
     * 
     * @param aParameter
     * @param aAtdl4jConfig
     * @return
     */
    public static int getDefaultDigitsForSpinnerControl(ParameterT aParameter,
	    Atdl4jConfig aAtdl4jConfig) {
	// -- not specified via rule above, use default if we have one within
	// Atdl4jConfig --
	if (aAtdl4jConfig != null) {
